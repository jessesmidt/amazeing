...
def parse_config(filename: str) -> dict:
    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            if len(parts) != 2:
                raise ValueError(
                    f"Invalid config line format: '{line}'. Expected KEY=VALUE"
                    )
    ...
    try:
        ...
        if len(entry_parts) != 2:
            raise ValueError(
                f"ENTRY must be in format 'x,y', got: {config['ENTRY']}"
                )
        ...
        if 'PATTERN' in config:
            ...
            if not pattern.isalnum():
                raise ValueError(
                    "PATTERN must be alphanumeric (letters and numbers only)"
                    )
